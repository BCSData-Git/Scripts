USE [Maintenance]
GO
SELECT * FROM mntblSQLExceptionLog_Header 
SELECT * FROM mntblSQLExceptionLog_Detail

SELECT * FROM "dbo"."User" WHERE FirstName = 'Spencer'

/****** Object:  Table [dbo].[mntblSQLExceptionLog_Detail]    Script Date: 8/16/2016 10:53:23 PM ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  Table [dbo].[mntblSQLExceptionLog_Header]    Script Date: 8/16/2016 10:53:16 PM ******/
SET ANSI_NULLS ON
GO
...
USE [Maintenance]
GO
/****** Object:  StoredProcedure [dbo].[mnt_sp_ProcessErrorCapture]    Script Date: 8/16/2016 10:53:37 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
Select @DateTimeLast = fDatetimeEnd
From mntblSQLExceptionLog_Header
Where fReportID = (Select MAX(fReportID) From mntblSQLExceptionLog_Header with (nolock))
...
Create table #Report (fReportID INT, [ErrorTime] [datetime] NOT NULL, [ServerName] [nvarchar](100) NOT NULL, [ClientHostName] [nvarchar](100) NULL
	, [DatabaseID] [int] NULL, [UserName] [nvarchar](100) NULL,	[ErrorSeverity] [bigint] NULL, [ErrorNumber] [bigint] NULL,	[Error] [nvarchar](512) NULL
	, [ErrorMessage] [nvarchar](512) NULL, [SQLText] [nvarchar](max) NULL, [EventData] [xml] NULL) 
...
where err_message is not null
  and [err_timestamp] >= @DateTimeLast
...
Insert into [dbo].[mntblSQLExceptionLog_Detail] (fReportID, ErrorTime, ServerName, ClientHostName, DatabaseID, UserName, ErrorSeverity, ErrorNumber, Error
	, ErrorMessage, SQLText, EventData)
Select @ReportID, ErrorTime, ServerName, ClientHostName, DatabaseID, UserName, ErrorSeverity, ErrorNumber, Error
	, ErrorMessage, SQLText,[EventData]
from #Report

GO

--GRANT EXEC on mnt_sp_ProcessErrorCapture to fxMonitor_role
--GO
...
IF EXISTS(SELECT * FROM sys.server_event_sessions WHERE name='ErrorCapture') 
...
(
    ACTION
    (   --sqlserver.server_instance_name, -- Available on SQL server 2012
...
(
    SET filename=N'C:\MSSQL\dbEvents\VSQLSERVER_ErrorCapture.xel', max_file_size=(2) -- Size in MB
)
WITH 
(
    MAX_MEMORY=4096 KB,
