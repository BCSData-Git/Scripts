USE [Maintenance]
GO
/****** Object:  StoredProcedure [dbo].[mnt_sp_Purge_montblXE_CPUReadsFilterTrace_Consol]    Script Date: 10/11/2022 3:13:20 PM ******/
SET ANSI_NULLS ON
GO
...
-- Description:	Purge table data for montblXE_CPUReadsFilterTrace_Consol
--
-- Sample:  Exec mnt_sp_Purge_montblXE_CPUReadsFilterTrace_Consol @retentionDays = 15 --(15 days testing only)
-- =============================================

CREATE PROC [dbo].[mnt_sp_Purge_montblXE_CPUReadsFilterTrace_Consol]

	@retentionDays smallint = 30
...
BEGIN
	...
	--select top 100 * from montblXE_CPUReadsFilterTrace_Consol
	--SP_HELP montblXE_CPUReadsFilterTrace_Consol
	--SP_HELPINDEX montblXE_CPUReadsFilterTrace_Consol
	...
	WHERE [timestamp] < GETUTCDATE() -@retentionDays
	...
	BEGIN --Loop
		...
		From #ToDelAll a
			--Join montblXE_CPUReadsFilterTrace_Consol c with (nolock) on a.ConsolId = c.ConsolId

		--Delete batch from prod table 
		Delete c
		From #ToDelBatch b 
			Join montblXE_CPUReadsFilterTrace_Consol c On b.ConsolId = c.ConsolId 
